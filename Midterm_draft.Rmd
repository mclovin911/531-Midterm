...
```{r}
anhist_low <- ts(loess(anhist~as.numeric(drugs$datum), span=0.5)$fitted, start=2014, frequency=12)
anhist_high <- ts(anhist - loess(anhist~as.numeric(drugs$datum), span=0.1)$fitted, start=2014, frequency=12)
